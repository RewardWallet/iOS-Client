...
final class AccountHeaderViewCell: RWCollectionReusableView {
    ...
    private let nameLabelBackground: GradientView = {
        ...
        view.colors = [.primaryColor, UIColor.primaryColor.darker()]
...
extension AccountHeaderViewCell: ListBindable {
    ...
    func bindViewModel(_ viewModel: Any) {
        ...
        guard let urlString = user.profileImage?.url, let url = URL(string: urlString) else { return }
        profileImageView.kf.indicatorType = .activity
        profileImageView.kf.setImage(with: url) { [weak self] (image, _, _, _) in
