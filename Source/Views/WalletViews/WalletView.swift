...
open class WalletView: UIView, UIScrollViewDelegate {
    
    public typealias PresentedCardViewDidUpdateBlock    = (CardView?) -> Void
    public typealias LayoutCompletion                   = (Bool) -> Void
    public typealias InsertionCompletion                = () -> Void
    public typealias RemovalCompletion                  = () -> Void
    public typealias RemovalAnimation                   = (CardView, RemovalCompletion?) -> Void
    ...
    /// Number of card views to show in the bottom of the wallet view when presenting card view. Default value is 5
    open var maximimNumberOfCollapsedCardViewsToShow: Int = 5 { didSet { calculateLayoutValues() } }
    
    ///  The positioning of card views relative to each other when the wallet view is not presenting a card view.  Default value is 52.
    open var minimalDistanceBetweenStackedCardViews: CGFloat = 52 { didSet { calculateLayoutValues() } }
    
    ///  The positioning of card views relative to each other when the wallet view is presenting a card view.  Default value is 8.
    open var minimalDistanceBetweenCollapsedCardViews: CGFloat = 8 { didSet { calculateLayoutValues() } }
    
    ///  The positioning of card views relative to the receiver’s presenting card view.
    open var distanceBetweetCollapsedAndPresentedCardViews: CGFloat = 10 { didSet { calculateLayoutValues() } }
    
    ///  The pop up offset of a card view when a long tap detected. Default value is 26.
    open var grabPopupOffset: CGFloat = 26 { didSet { calculateLayoutValues() } }
    
    ///  The total duration of the animations when the card view is being presented. Default value is 0.3
    open var presentingAnimationSpeed: TimeInterval = 0.3
    
    ///  The total duration of the animations when the card view is being dismissed. Default value is 0.3
    open var dismissingAnimationSpeed: TimeInterval = 0.3
    
    ///  The total duration of the animations when the card view is being insertred. Default value is 0.5
    open var insertionAnimationSpeed: TimeInterval = 0.5
    
    ///  The total duration of the animations when the card view is being removed. Default value is 0.5
    open var removalAnimationSpeed: TimeInterval = 0.5
    
    ///  The total duration of the animations when the card view is being grabbed. Default value is 0.15
    open var grabbingAnimationSpeed: TimeInterval = 0.15
    
    ///  This block is called after the receiver’s card view is presented or dimissed.
    open var didUpdatePresentedCardViewBlock: PresentedCardViewDidUpdateBlock?
    
    ///  Returns an accessory view that is displayed above the wallet view.
    public weak var walletHeader: UIView? {
        willSet {
    ...
    /// The current CardView that is being grabbed
    private(set) public weak var grabbedCardView: CardView?
    
    // MARK: - Properties [Private]
    
    private let scrollView: UIScrollView = {
        let scrollView = UIScrollView()
        scrollView.bounces = true // seems to increase performance
    ...
    private var walletHeaderHeight:             CGFloat = 0
    private var cardViewTopInset:               CGFloat = 0
    private var maximumCardViewHeight:          CGFloat = 0
    private var cardViewHeight:                 CGFloat = 0
    private var distanceBetweenCardViews:       CGFloat = 0
    
    private var presentationCenter: CGPoint {
        let centerRect = CGRect(x: 0, y: cardViewTopInset, width: frame.width, height: frame.height - collapsedCardViewStackHeight - cardViewTopInset)
    ...
    /// Adds the required observers to the scrollView
    ...
    /// Override to add additional setup
    ...
    open func reload(cardViews: [CardView]) {
        ...
        insertedCardViews.forEach { $0.removeFromSuperview() }
    ...
    open func present(cardView: CardView, animated: Bool, completion: LayoutCompletion? = nil) {
        ...
        present(cardView: cardView, animated: animated, animationDuration: animated ? presentingAnimationSpeed : nil, completion: completion)
    }
    ...
    open func dismissPresentedCardView(animated: Bool, completion: LayoutCompletion? = nil) {
        ...
        dismissPresentedCardView(animated: animated, animationDuration: animated ? dismissingAnimationSpeed : nil, completion: completion)
    }
    ...
    open func insert(cardView: CardView, animated: Bool = false, presented: Bool = false,  completion: InsertionCompletion? = nil) {
        ...
        if animated {
            ...
            DispatchQueue.main.async {
                UIView.animateKeyframes(withDuration: self.insertionAnimationSpeed, delay: 0, options: [.beginFromCurrentState, .calculationModeCubic], animations: { [weak self] in
    ...
    /// Transitions the supplied cardView to the grabbeCardView
    ///
    /// - Parameters:
    ///   - cardView: The CardView to transition
    ///   - popup: If the card should be popped up to full presentation context
    public func grab(cardView: CardView, popup: Bool) {
        ...
        DispatchQueue.main.async {
            UIView.animate(withDuration: self.grabbingAnimationSpeed, delay: 0, options: [.beginFromCurrentState, .curveEaseInOut], animations: { [weak self] in
    ...
    /// Updates the grabbedCardView's offset
    ///
    /// - Parameter offset: new y offset for the CardView
    public func updateGrabbedCardView(offset: CGFloat) {
    ...
    /// Transitions the current grabbedCardView to be back in the stack
    public func releaseGrabbedCardView() {
        ...
        if let grabbedCardView = grabbedCardView,
            ...
            layoutWalletView(animationDuration: grabbingAnimationSpeed)
        }
    }
    
    // MARK: Private methods
    
    private func insert(cardViews: [CardView]) {
        
        insertedCardViews = cardViews
        if insertedCardViews.count == 1 {
            presentedCardView = insertedCardViews.first
        }
    }
    
    private func present(cardView: CardView, animated: Bool, animationDuration: TimeInterval?, completion: LayoutCompletion? = nil) {
    ...
    private func dismissPresentedCardView(animated: Bool, animationDuration: TimeInterval?, completion: LayoutCompletion? = nil) {
    ...
    private func removalAnimation() -> RemovalAnimation {
        ...
        return { [weak self] (cardView: CardView,  completion: RemovalCompletion?) in
            ...
            DispatchQueue.main.async {
                UIView.animateKeyframes(withDuration: self?.removalAnimationSpeed ?? 0.3, delay: 0, options: [.calculationModeCubic], animations: {
    ...
    private func calculateLayoutValues(shouldLayoutWalletView: Bool = true) {
    ...
    private func layoutWalletHeader() {
    ...
    private func layoutWalletView(animationDuration: TimeInterval? = nil,
        ...
        for cardViewIndex in 0..<insertedCardViews.count {
            ...
            if cardView == firstCardView {
                ...
                if let stretchingDistanse = stretchingDistanse {
                    cardViewFrame.origin.y += stretchingDistanse
                }
                ...
                if let stretchingDistanse = stretchingDistanse {
                    cardViewFrame.origin.y += stretchingDistanse * CGFloat(cardViewIndex)
                }
    ...
    private func makeCollapseLayout(collapsePresentedCardView: Bool = false) {
