...
final class ActionSheetController: UIViewController, ListAdapterDataSource, ListSingleSectionControllerDelegate {
    ...
    private let cancelToken = Action(title: "Cancel", style: .cancel, callback: nil)
    ...
    func listAdapter(_ listAdapter: ListAdapter, sectionControllerFor object: Any) -> ListSectionController {
        let configBlock: ListSingleSectionCellConfigureBlock = { (object, cell) in
            guard let actionCell = cell as? ActionSheetCell, let action = object as? Action else { return }
    ...
    func didSelect(_ sectionController: ListSingleSectionController, with object: Any) {
        if let action = object as? Action {
