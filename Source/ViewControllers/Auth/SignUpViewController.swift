...
final class SignUpViewController: RWViewController {
    ...
    private func didTapRegister() {
        ...
        User.signUpInBackground(email: email, password: password) { [weak self] (success, error) in
            ...
            if let user = User.current() {
                if let num = Int(self?.phoneField.text?.digits ?? "") {
                    user.phone = NSNumber(value: num)
                }
